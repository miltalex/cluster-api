...
func (r *Reconciler) reconcileMachineDeployments(ctx context.Context, s *scope.Scope) error {
	...
	// Track MachineDeployments that are already rolling out before deciding whether additional
	// MachineDeployments can start a rollout in this reconcile.
...
func (r *Reconciler) updateMachineDeployment(ctx context.Context, s *scope.Scope, mdTopologyName string, currentMD, desiredMD *scope.MachineDeploymentState) error {
	...
	if wouldTriggerRollout && !s.UpgradeTracker.MachineDeployments.IsRollingOut(currentMD.Object.Name) {
		if s.UpgradeTracker.MachineDeployments.RolloutConcurrencyReached() {
