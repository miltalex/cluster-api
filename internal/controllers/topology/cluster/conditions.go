...
func (r *Reconciler) reconcileTopologyReconciledCondition(s *scope.Scope, cluster *clusterv1.Cluster, reconcileErr error) error {
	...
	if isUpgrading || isRolloutPending {
		...
		if isRolloutPending {
			fmt.Fprintf(msgBuilder, "\n  * %s rollout pending because the rollout concurrency limit has been reached", nameList("MachineDeployment", "MachineDeployments", s.UpgradeTracker.MachineDeployments.PendingRolloutNames()))
		}
